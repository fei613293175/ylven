...
func TestMultimodalProbeIsExplicitWhenUnconfigured(t *testing.T){result:=Client{}.ProbeMultimodal(context.Background());if result.Status!=Unconfigured||result.ErrorCode!="SUB2API_NOT_CONFIGURED"{t.Fatalf("%#v",result)}}
