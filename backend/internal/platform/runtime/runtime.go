...
func Handler(service Service, featurePath string) http.Handler {
	...
	if service.Dependencies == nil {
		service.Dependencies = map[string]bool{}
	}
	mux := http.NewServeMux()
	health := func(w http.ResponseWriter, r *http.Request) {
		writeHealth(w, service, r.URL.Path == "/readyz" || r.URL.Path == "/health/ready")
	}
	...
	mux.HandleFunc(featurePath, func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			writeError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "method not allowed", false)
			return
		}
