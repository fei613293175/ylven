...
export default {
  setup() {
    ...
    const actionLabel = computed(() => path.value === '/admin/037' ? '新增角色' : path.value === '/admin/040' ? '编辑模板' : path.value === '/admin/039' ? '安全确认' : ['/admin/026','/admin/034','/admin/035'].includes(path.value) ? '编辑配置' : '')
    ...
    async function load() {
      data.value = null; error.value = ''; notice.value = ''; roleEditor.value = false; templateDraft.value = null; settingDraft.value = null; settingEditor.value = false; selectedUser.value = null
      if (!token.value || !current.value.endpoint) return
      loading.value = true
      try { data.value = await parseResponse(await fetch(current.value.endpoint, { headers: authHeaders() })); if (data.value.templates?.length) templateDraft.value = { ...data.value.templates[0] }; if (data.value.value) settingDraft.value = { ...data.value.value } }
      catch (reason) { error.value = reason.message }
      finally { loading.value = false }
    }
    function runAction() { if (path.value === '/admin/037') roleEditor.value = true; else if (path.value === '/admin/039') stepUpOpen.value = true; else if (path.value === '/admin/040') document.querySelector('.template-subject')?.focus(); else if (settingDraft.value) settingEditor.value = true }
    ...
    return { navItems, path, token, username, credential, stepUpCredential, stepUpToken, stepUpOpen, roleEditor, role, templateDraft, settingDraft, settingEditor, selectedUser, current, allowed, actionLabel, loading, saving, data, error, notice, navigate, navActive, login, logout, load, runAction, confirmStepUp, updateUser, createRole, saveTemplate, saveSetting, openUser }
  },
  template: `
